...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"yocstats", "explorer", "wallet", "faucet"} {
	...
	// If we have yocstats running, ask whether to make the secret public or not
	if w.conf.yocstats != "" {
		fmt.Println()
		fmt.Println("Include yocstats secret on dashboard (y/n)? (default = yes)")
