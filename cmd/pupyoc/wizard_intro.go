...
// makeWizard creates and returns a new yoceth wizard.
...
// setting up a new or managing an existing YOC private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to Yoceth, your YOC private network manager       |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new YOC network down to       |")
	fmt.Println("| the genesis block, bootnodes, miners and yocstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Yoceth uses SSH to dial in to remote servers, and builds  |")
	...
	log.Info("Administering YOC network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".yoceth", w.network)
