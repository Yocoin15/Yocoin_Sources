...
ADD yocstats.json /yocstats.json
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	yocstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(yocstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.yocstats[:strings.Index(config.yocstats, ":")],
		"Secret": config.yocstats[strings.Index(config.yocstats, ":")+1 : strings.Index(config.yocstats, "@")],
		"Host":   config.yocstats[strings.Index(config.yocstats, "@")+1:],
	})
	files[filepath.Join(workdir, "yocstats.json")] = yocstats.Bytes()
