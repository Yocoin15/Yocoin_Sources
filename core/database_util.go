...
func WriteCanonicalHash(db yocdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db yocdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db yocdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db yocdb.Putter, hash common.Hash) error {
...
func WriteHeader(db yocdb.Putter, header *types.Header) error {
...
func WriteBody(db yocdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db yocdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db yocdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db yocdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db yocdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db yocdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db yocdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db yocdb.Database) yocdb.Database {
	return yocdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db yocdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db yocdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db yocdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
