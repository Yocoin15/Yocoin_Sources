...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*yocdb.MemDatabase).Keys() {
	...
	statedb, _ := yocdb.NewMemDatabase()
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	if !timeout {
		blocks, _ := core.GenerateChain(&params.ChainConfig{}, genesis, yochash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
			if remoteForked {
