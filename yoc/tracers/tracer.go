...
type Tracer struct {
	inited bool // Flag whether the context was already inited from the YVM
...
func (jst *Tracer) CaptureState(env *vm.YVM, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
...
func (jst *Tracer) CaptureFault(env *vm.YVM, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
