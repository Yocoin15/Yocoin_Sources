...
// TraceBlock returns the structured logs created during the execution of YVM
...
// TraceTransaction returns the structured logs created during the execution of YVM
// and returns them as a JSON object.
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	vmenv := vm.NewYVM(vmctx, statedb, api.config, vm.Config{Debug: true, Tracer: tracer})
	...
	case *vm.StructLogger:
		return &yocapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  yocapi.FormatLogs(tracer.StructLogs()),
		}, nil
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int, reexec uint64) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		vmenv := vm.NewYVM(context, statedb, api.config, vm.Config{})
